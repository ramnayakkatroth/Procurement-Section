...
let factory;
let campaign;
//console.log(compiledCampaignFactory.interface);
...
describe('Campaign',() => {
  ...
  it('has minimum contribution', async () => {
    const minimum = await campaign.methods.minumumContribution().call();
    assert(minimum == 100);
  });
  ...
  it('allow to finalize request', async ()=> {
    await campaign.methods.contribute().send({
    ...
    let balance = await web3.utils.fromWei(await web3.eth.getBalance(accounts[1]),'ether');
